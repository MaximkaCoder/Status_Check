...
export default function NewItemPage() {
  ...
  const [prefillDeadline, setPrefillDeadline] = useState<string | undefined>();
  const [formKey, setFormKey] = useState(0);

  function handleParsed(values: { title: string; deadline?: string }) {
    setPrefillTitle(values.title);
    if (values.deadline) {
      try {
        const d = new Date(values.deadline);
        setPrefillDeadline(format(d, "yyyy-MM-dd'T'HH:mm"));
      } catch {
        setPrefillDeadline(undefined);
      }
    } else {
      setPrefillDeadline(undefined);
    }
  ...
  const hasPrefill = !!(prefillTitle || prefillDeadline);
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8">
      ...
      {activeTab === "smart" && (
        <div
          className="rounded-2xl border border-border/60 bg-card shadow-card p-6 animate-scale-in"
        >
